...
// Determine if the new position that will try to occupate is free and move to it
	private void mover()
	{
...
// Critical area: Check if the position is occupated. If find it in the objPosiciones, the position it tries to get is occupated, so it gets locked by position semaphore
...
// Movement inside the mine. No locking because there's a lock in the vein starting position
...
// If it's a Minero, go to the vein position
				...
				break;
			case TIPO_TREN:
// If it's a Tren, go to the wait train point and wait until Mineros drops at least 120 beepers
				pasosEntradaComunesMineroTren();
				ingresoTren();
				break;
			case TIPO_EXTRACTOR:
// If it's a Extractor, waits until the trains drops beepers in the delivery point
				if(!inicioTransporte)
				{
					try{
...
// Minero and Tren robots use the same steps to enter until they get to Avenue 8 Street 11
...
// Miners start the Trains when the last Miner arrives to Avenue 1
		if(tipoRobot == TIPO_MINERO)
		{
			...
			{
				try{
					ejecutarLog = (debugHabilitado) ? logMensaje("Solo el último minero puede activar los trenes") : false;
					sem_salida.acquire();
				}
				catch(InterruptedException exc)
				{
...
// Trains start the Extractors when the last Train arrives to Avenue 1
		if(tipoRobot == TIPO_TREN)
		{
			...
			{
				try{
					ejecutarLog = (debugHabilitado) ? logMensaje("Solo el último tren puede activar los extractores") : false;
					sem_salida.acquire();
				}
				catch(InterruptedException exc)
				{
		...
		irAlMuro();
// At Avenue 1 Street 8, the first Extractor enters and the rest waits until the first exit from the mine.
		if(tipoRobot == TIPO_EXTRACTOR)
		{
			try{
...
// The first one, enters direct to the vein. Second one that arrives, goes to the waiting point.
...
// Trains goes to the waiting point and waits until the Miners delivers at least 120 beepers
// controlled by the sem_trenInicioProceso
	private void ingresoTren()
	{
		...
		{
			try{
				ejecutarLog = (debugHabilitado) ? logMensaje("Espero que hayan " + BEEPERS_TREN + " beepers para recogerlos.") : false;
...
// Extractor moves to the pick point where the Trains drops the 120 beepers per trip
	...
	private void procesar()
	{
		...
		switch(tipoRobot)
		{
			case TIPO_MINERO:
				...
				break;
			case TIPO_TREN:
// Train process maintains while Miners finds beepers in the vein and left them in the drop point
				while(nextToABeeper() || !minaVacia)
					procesarTren();
				turnRight();
				break;
			case TIPO_EXTRACTOR:
// Extractor process maintains while Trains finds beepers in the drop point and left them in the drop point
...
// Steps to go from mine entrance to the Extractor Waiting point. All robots must do this instructions.
...
// With the amount of beepers found it in the vein delivery point, goes to the extractors delivery point at Avenue 3 Street 1
	private void irAlPuntoDeEntrega()
	{
		...
		irAlMuro();
		turnLeft();
// If there's any beeper in its bag, go to the delivery point at Avenue 3 Street 1. If not, moves or locks until Miner informs that the vein is empty.
...
// Trains aren't working because they didn´t have beepers to pick. So, miners allows them to move.
			ejecutarLog = (debugHabilitado) ? logMensaje("Me muevo") : false;
			mover();
			...
			if(!inicioExtraccion && beepersExtraidos >= BEEPERS_TREN)
			{
				...
				ejecutarLog = (debugHabilitado) ? logMensaje("" + sem_trenInicioProceso.availablePermits() + " <= " + (cantidadTrenes - 1)) : false;
...
// Unload beepers in the vein delivery point
			descargar();
			ejecutarLog = (debugHabilitado) ? logMensaje("No está la mina vacia o si está y no ha terminado de extraer?") : false;
...
// No beepers on the Vein. So I wait until the beepers went fully to the warehouses
...
// Miners puts beepers in the vein dropping point. If they have at least 120 dropped, release the trains from the waiting point
...
// All robots do this to leave the mine from the extracting point at Avenue 3 Street 1
		pasosSalidaComunes();
	}

// Route from the extracting point at Avenue 3 Street 1 to the main entrance of the Mine
...
// All robots in the mine do this, except the robot in the Warehouse
		if(tipoRobot <= TIPO_TREN || (tipoRobot == TIPO_EXTRACTOR && calleActual == CALLE_ESPERA_EXT && avenidaActual == AVENIDA_ESPERA_EXT))
			salirMina();
		...
		if(args.length > 0)
		{
			for(int i = 0; i < args.length; i++)
			{
				...
				else
				{
					...
					{
						System.out.println("Se esperaba un numero para el comando " + args[0] + ". El programa termina.");
						System.exit(2);
					}
...
// Check if the position requested is already occupated by a robot. If so, locks the thread.
